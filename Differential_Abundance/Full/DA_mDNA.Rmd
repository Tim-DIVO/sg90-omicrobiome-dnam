...
# Define the volcano plot function using ggplot2
create_volcano_plot_gg <- function(data, diff_col, pval_col, output_file, 
                                   pval_threshold = 0.05, 
  ...
  # Determine y-axis upper limit with some padding
  y_max <- ceiling(max(data$neg_log10_pval, na.rm = TRUE)) + 1
  
# Create the plot
p <- ggplot(data, aes_string(x = diff_col, y = "neg_log10_pval")) +
  geom_point(aes(color = Significant), alpha = 0.7, size = 2, show.legend = TRUE) +
  scale_color_manual(
    values = c("FALSE" = non_sig_color),
    labels = c("Not Significant")
  ) +
  geom_hline(yintercept = -log10(pval_threshold), linetype = "dashed", color = "black") +
  labs(
    x = "Log2(difference between EAD groups)",
    y = "-log10(p-value)",
    color = ""
  ) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.3)) +
  guides(color = guide_legend(override.aes = list(
    shape = 16,   # Shape of the point in the legend
    color = non_sig_color  # Color of the point in the legend
  ))) +
  ...
  ggsave(filename = output_file, plot = p, width = 8, height = 6, dpi = 600, units = "in")
}
...
create_volcano_plot_gg(
  data = asv_all,
  diff_col = "diff.btw",
  pval_col = "we.eBH",
  output_file = file.path(output_dir_gg, "volcano_ASV_gg.png")
)
...
create_volcano_plot_gg(
  ...
  diff_col = "diff.btw",
  pval_col = "we.eBH",
  output_file = file.path(output_dir_gg, "volcano_Genus_gg.png")
)
...
create_volcano_plot_gg(
  ...
  diff_col = "diff.btw",
  pval_col = "we.eBH",
  output_file = file.path(output_dir_gg, "volcano_KO_gg.png")
)
...
create_volcano_plot_gg(
  ...
  diff_col = "diff.btw",
  pval_col = "we.eBH",
  output_file = file.path(output_dir_gg, "volcano_Pathway_gg.png")
)
