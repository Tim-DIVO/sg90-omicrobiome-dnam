...
se.mb <- spiec.easi(list(MB_matrix, Cpg_matrix), method='mb', lambda.min.ratio=5e-2, nlambda=20, pulsar.params=list(rep.num=50, ncores=num_cores))
saveRDS(se.mb, file = "se_5e2_full_mb.rds")    
...
se.gl <- spiec.easi(list(MB_matrix, Cpg_matrix), method='glasso', lambda.min.ratio=5e-2, nlambda=20, pulsar.params=list(rep.num=50, ncores=num_cores))
saveRDS(se.gl, file = "se_5e2_full_gl.rds")
